...
proptest! {
    ...
    fn required_signed_bits_prop(
        ...
        let abs = InternalArbiUint::from_limbs(limbs);
        let pos = is_positive || abs.is_zero();
        let val = InternalArbiInt {
            abs,
            is_positive: pos,
        };
