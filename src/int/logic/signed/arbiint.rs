...
#[derive(Debug, Clone)]
pub struct InternalArbiInt {
    /// The unsigned magnitude of the integer.
    pub(crate) abs: InternalArbiUint,
...
impl InternalArbiInt {
    ...
    pub fn with_capacity(limbs: usize) -> Self {
        Self {
            abs: InternalArbiUint::with_capacity(limbs),
            is_positive: true,
        }
    }
    ...
    #[must_use]
    pub fn to_tc_bits(&self, width: usize) -> InternalArbiUint {
    ...
    /// Constructs an `InternalArbiInt` from a two's-complement unsigned integer of `width` bits.
    #[must_use]
    pub fn from_tc_bits(bits: InternalArbiUint, width: usize) -> Self {
        debug_assert!(width > 0, "two's-complement width must be non-zero");
