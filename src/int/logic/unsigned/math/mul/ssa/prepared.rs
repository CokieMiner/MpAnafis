...
use super::{
    FftPlan, LIMB_BITS, Limb, SSA_BASE_MODULUS_BITS, Ssa, SsaCarry, SsaCrt, SsaPlan, SsaRing,
...
impl<'operands> SsaMultiplicationPlan<'operands> {
    ...
    pub fn try_new(
        ...
        let (strategy, scratch_len) = if use_direct_fermat {
            ...
            let slots = direct_plan.parallel_slots(parallelism.get());
            let direct_transform_scratch = direct_plan.transform_mul_scratch_for_slots(slots);
            ...
            let ring_plan = choice.forced_exponent().map_or_else(
    ...
    /// must remain usable for synchronous execution with the parallelism used to
    /// construct this plan.
    ...
    unsafe fn run_prepared_with_scratch<E: ParallelExecutor>(
        ...
        match strategy {
            SsaProductStrategy::DirectFermat {
                ...
                let output_len = self.result_len.min(ml);
                // SAFETY: result_len <= dst.len(), and output_len is also bounded
                // by the complete direct-ring data width.
                ...
                unsafe { dst.get_unchecked_mut(output_len..) }.fill(0);
            }
            SsaProductStrategy::CrtTwoModuli {
                n,
                ring_bits,
