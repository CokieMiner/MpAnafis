...
unsafe fn dif_radix4_stage(
    ...
    for (((u0, u1), u2), u3) in q0_chunks.zip(q1_chunks).zip(q2_chunks).zip(q3_chunks) {
        ...
        if w2 != 0 {
            if w2 == mod_bits {
                // SAFETY: u1 has cl limbs.
                unsafe {
                    SsaRing::negate(u1, mod_bits);
                }
            } else {
                // SAFETY: scratch_slot and u1 are disjoint cl-limb spans.
                unsafe {
                    SsaRing::shift_from(scratch_slot, u1, w2, mod_bits);
                }
                u1.copy_from_slice(scratch_slot);
            }
        }
        ...
        if w1 != 0 {
            if w1 == mod_bits {
                // SAFETY: u2 has cl limbs.
                unsafe {
                    SsaRing::negate(u2, mod_bits);
                }
            } else {
                // SAFETY: scratch_slot and u2 are disjoint cl-limb spans.
                unsafe {
                    SsaRing::shift_from(scratch_slot, u2, w1, mod_bits);
                }
                u2.copy_from_slice(scratch_slot);
            }
        }
        ...
        if w3 != 0 {
            if w3 == mod_bits {
                // SAFETY: u3 has cl limbs.
                unsafe {
                    SsaRing::negate(u3, mod_bits);
                }
            } else {
                // SAFETY: scratch_slot and u3 are disjoint cl-limb spans.
                unsafe {
                    SsaRing::shift_from(scratch_slot, u3, w3, mod_bits);
                }
                u3.copy_from_slice(scratch_slot);
            }
        }
