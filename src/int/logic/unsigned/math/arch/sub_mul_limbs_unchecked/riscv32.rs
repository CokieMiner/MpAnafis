//! RISC-V 32-bit fused multiply-subtract limb kernel.
//!
//! Uses 32×32→64-bit unsigned multipliers (`mul`/`mulhu`) and branchless
...
///
/// RISC-V 32-bit calculates 64-bit products via `mul`/`mulhu`, accumulates the multiplication
/// carry branchlessly with `sltu`, and performs two-stage subtraction borrow propagation using
/// `sltu` and `or`. The loop is 2-way unrolled (8 bytes per iteration).
///
/// # Safety
///
...
pub unsafe fn sub_mul_limbs_unchecked(
    ...
    unsafe {
        asm!(
            ...
            "mul {p_lo0}, {s0}, {scalar}",               // p_lo0 = low 32 bits of src[0] * scalar
            "mulhu {p_hi0}, {s0}, {scalar}",             // p_hi0 = high 32 bits of src[0] * scalar
