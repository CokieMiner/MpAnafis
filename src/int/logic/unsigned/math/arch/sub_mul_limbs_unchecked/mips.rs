...
//!
//! Uses 32×32→64-bit hardware multipliers (`multu`/`mflo`/`mfhi`), non-trapping addition/subtraction
...
///
/// MIPS 32-bit uses accumulator registers `HI` and `LO` for unsigned multiplication (`multu`).
/// Carries and borrows are tracked branchlessly without hardware condition flags using `sltu`
/// and non-trapping arithmetic `addu`/`subu`. The loop is 2-way unrolled (8 bytes per iteration).
///
/// # Safety
///
