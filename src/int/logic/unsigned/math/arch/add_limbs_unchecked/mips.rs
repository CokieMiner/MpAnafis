...
pub unsafe fn add_limbs_unchecked(dst: *mut Limb, src: *const Limb, len: usize) -> Limb {
    ...
    unsafe {
        asm!(
            ...
            "addu {t1}, {t1}, {t0}",       // t1 = dst + src (may wrap)
            "sltu {c0}, {t1}, {t0}",       // c0 = overflow from dst+src (t1 < t0)
            "addu {t1}, {t1}, {carry}",    // t1 += previous carry
            ...
            "addiu {src}, {src}, 16",      // advance src by 16 bytes (4 × u32)
            "addiu {dst}, {dst}, 16",      // advance dst by 16 bytes
            ...
            "or {carry}, {c0}, {c1}",      // combined carry for next limb
            "sw {t1}, 0({dst})",           // store result
            "addiu {src}, {src}, 4",       // advance src by 4 bytes
            "addiu {dst}, {dst}, 4",       // advance dst by 4 bytes
            "addiu {rem}, {rem}, -1",      // decrement remainder counter
