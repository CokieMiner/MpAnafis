//! RISC-V 64-bit addition kernels (inline assembly).
//!
...
//!
//! ```text
//!   add     tmp, dst, src           // tmp = dst + src  (may wrap)
//!   sltu    c0, tmp, src            // c0 = 1 if tmp < src  (overflow from dst+src)
//!   add     result, tmp, carry      // result = tmp + carry
...
pub unsafe fn add_limbs_unchecked(dst: *mut Limb, src: *const Limb, len: usize) -> Limb {
    ...
    unsafe {
        asm!(
            ...
            "addi {src}, {src}, 32",       // advance src by 32 bytes (4 × u64)
            ...
            "or {carry}, {c0}, {c1}",      // combined carry for next limb
            "sd {t1}, 0({dst})",           // store result
            "addi {src}, {src}, 8",        // advance src by 8 bytes
