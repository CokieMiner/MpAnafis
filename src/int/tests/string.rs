...
fn to_string_radix_rejects_every_invalid_boundary_before_zero_fast_path() {
    let unsigned_zero = MpUint::zero();
    let unsigned_nonzero = MpUint::from(37_u8);
    let signed_zero = MpInt::zero();
    let signed_nonzero = MpInt::from(-37_i8);
...
fn power_of_two_formatting_zero_extends_the_final_partial_digit() {
    ...
    let value = MpUint::from(1_152_921_504_606_846_976_u64);
...
fn base8_formatting_partial_top_block() {
    ...
    let value = MpUint::from(2_305_843_009_213_693_952_u64);
...
fn base32_formatting_exact_full_top_block() {
    ...
    let value = MpUint::from(549_755_813_888_u64);
...
fn base8_formatting_exact_full_top_block() {
    ...
    let value = MpUint::from(8_388_608_u64);
...
fn decimal_chunk_boundaries_are_intact() {
    ...
    let below_19 = MpUint::from(999_999_999_999_999_999_u64);
    assert_eq!(below_19.to_string(), "999999999999999999");

    let at_10_18 = MpUint::from(1_000_000_000_000_000_000_u64);
    assert_eq!(at_10_18.to_string(), "1000000000000000000");

    let at_10_19 = MpUint::from(10_000_000_000_000_000_000_u128);
    assert_eq!(at_10_19.to_string(), "10000000000000000000");

    let just_above = MpUint::from(10_000_000_000_000_000_001_u128);
...
fn decimal_chunk_boundaries_for_wide_chunks() {
    ...
    let below = MpUint::from(10_000_000_000_000_000_000_u128)
        * MpUint::from(10_000_000_000_000_000_000_u128)
        - MpUint::from(1_u8);
    assert_eq!(below.to_string(), "99999999999999999999999999999999999999");

    let exact = MpUint::from(10_000_000_000_000_000_000_u128)
        * MpUint::from(10_000_000_000_000_000_000_u128);
    assert_eq!(exact.to_string(), "100000000000000000000000000000000000000");

    let u128_max = MpUint::from(u128::MAX);
    assert_eq!(
        u128_max.to_string(),
...
6801874298166903427690031858186486050853753882811946569946433649006084095"
    ...
    let round_trip = MpUint::from_str_radix(&rendered, 10).expect("parse radix 10");
...
proptest! {
    ...
    fn prop_roundtrip_base8_base32(value in strategies::uint(8)) {
        for radix in [8_u32, 32] {
            ...
            let round_trip = MpUint::from_str_radix(&digits, radix)
...
proptest! {
    ...
    fn prop_string_format_roundtrip(value in strategies::int_maybe_bounded(8)) {
        ...
        let decimal_round_trip = MpInt::from_str_radix(&decimal, 10).expect("radix 10");
        prop_assert_eq!(&decimal_round_trip, &value, "radix 10");

        let binary = format!("{value:b}");
        let binary_round_trip = MpInt::from_str_radix(&binary, 2).expect("radix 2");
        prop_assert_eq!(&binary_round_trip, &value, "radix 2");

        let octal = format!("{value:o}");
        let octal_round_trip = MpInt::from_str_radix(&octal, 8).expect("radix 8");
        prop_assert_eq!(&octal_round_trip, &value, "radix 8");

        let lower_hex = format!("{value:x}");
        let lower_hex_round_trip = MpInt::from_str_radix(&lower_hex, 16).expect("radix 16");
        prop_assert_eq!(&lower_hex_round_trip, &value, "lower hex");

        let upper_hex = format!("{value:X}");
        let upper_hex_round_trip = MpInt::from_str_radix(&upper_hex, 16).expect("radix 16");
...
proptest! {
    ...
    fn prop_roundtrip_hex(value in strategies::uint(8)) {
        ...
        let round_trip = MpUint::from_str_radix(&hex, 16).expect("parse hex");
    ...
    fn prop_roundtrip_decimal(value in strategies::uint(6)) {
        ...
        let round_trip: MpUint = decimal.parse().expect("parse decimal");
...
proptest! {
    ...
    fn prop_string_parse_errors(
        ...
        let empty_err = MpUint::from_str_radix("", 10).expect_err("should fail");
        prop_assert_eq!(
            empty_err.to_string(),
            "cannot parse integer from empty string"
        );

        let valid_digits = MpUint::from(magnitude).to_string_radix(radix);
        let invalid_digits = format!("{valid_digits}_");
        let char_err =
            MpUint::from_str_radix(&invalid_digits, radix).expect_err("should fail");
        prop_assert_eq!(char_err.to_string(), "invalid digit found in string");

        let radix_err =
            MpUint::from_str_radix(&valid_digits, invalid_radix).expect_err("should fail");
        prop_assert_eq!(radix_err.to_string(), "invalid radix");

        let negative_digits = format!("-{valid_digits}");
        let neg_err =
            MpUint::from_str_radix(&negative_digits, radix).expect_err("should fail");
        prop_assert_eq!(
            neg_err.to_string(),
            "cannot parse unsigned integer from negative value"
        );

        let empty_signed_err = MpInt::from_str_radix("-", 10).expect_err("should fail");
        prop_assert_eq!(
        ...
        let invalid_char_int = MpInt::from_str_radix(&invalid_signed_digits, radix)
...
proptest! {
    ...
    fn prop_string_parse_leading_zeros(value in any::<u64>()) {
        ...
        let parsed = MpUint::from_str_radix(&encoded, 10).expect("should succeed");
        prop_assert_eq!(parsed, MpUint::from(value), "uint leading zeros parse failed for {}", value);
    }
}
...
proptest! {
    ...
    fn prop_string_parse_leading_zeros_signed(value in any::<i64>().prop_filter("non-min", |v| *v != i64::MIN)) {
        ...
        let parsed = MpInt::from_str_radix(&encoded, 10).expect("should succeed");
        prop_assert_eq!(parsed, MpInt::from(value), "signed leading zeros parse failed for {}", value);
    }
}
