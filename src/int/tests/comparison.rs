...
proptest! {
    ...
    fn prop_numeric_properties(a in strategies::uint(8)) {
        let is_even_mod = (&a % ArbiUint::from(2_u8)).is_zero();
        ...
        let i_pos = ArbiInt::from(a.clone());
...
proptest! {
    ...
    fn prop_new_public_apis_cmp(c1 in strategies::uint(16), c2 in strategies::uint(16), c3 in strategies::uint(16)) {
        let min_val = ArbiUint::min(c1.clone(), c2.clone());
        let max_val = ArbiUint::max(c1.clone(), c2.clone());
        let _clamp_val = ArbiUint::clamp(
            c1.clone(),
            ArbiUint::min(c2.clone(), c3.clone()),
            ArbiUint::max(c2.clone(), c3),
        );
