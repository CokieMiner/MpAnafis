...
proptest! {
    ...
    fn prop_byte_serialization_roundtrip(u: u64, i: i64) {
        ...
        prop_assert!(ArbiUint::from_le_bytes(&le_bytes_u) == au, "from_le_bytes");
        let be_bytes_u = au.to_be_bytes();
        prop_assert_eq!(ArbiUint::from_be_bytes(&be_bytes_u), au);

        let ai = ArbiInt::from(i);
        let le_bytes_i = ai.to_le_bytes();
        prop_assert!(ArbiInt::from_le_bytes(&le_bytes_i) == ai, "from_le_bytes");
        let be_bytes_i = ai.to_be_bytes();
        prop_assert_eq!(ArbiInt::from_be_bytes(&be_bytes_i), ai);
    }
}
