//! Tour of the `arbi-anafis` API across precision, bitwise,
...
use arbi_anafis::{ArbiInt, ArbiUint, BoundedPrecision, Precision};
...
fn arithmetic_semantics() {
    ...
    let max16 = ArbiUint::with_precision_checked(65_535_u64, bp).expect("u16 max");
    let one = ArbiUint::with_precision_wrapping(1_u64, bp);
    ...
    assert_eq!(
        ...
        ArbiUint::with_precision_wrapping(0_u64, bp),
    ...
    let word = ArbiUint::from(0x1234_5678_u64);
    let rev = word.reverse_bits(32).expect("32-bit reverse");
    assert_eq!(rev, ArbiUint::from(0x1E6A_2C48_u64), "reverse 32 bits");
    assert_eq!(
    ...
    let rot = ArbiUint::from(0xFF00_00FF_u64)
        .rotate_left(8, 32)
        .expect("32-bit rotate");
    assert_eq!(rot, ArbiUint::from(0x0000_FFFF_u64), "left-rotate 8 bits");
    println!("bitwise:            [OK] rotate_left");

    let sbp = BoundedPrecision::new(8).expect("8-bit precision");
    let neg = ArbiInt::with_precision_checked(-7_i64, sbp).expect("-7 fits in i8");
    let three = ArbiInt::from(3_i64);
    ...
    assert_eq!(neg.div_euclid(&three), ArbiInt::from(-3_i64), "div_euclid");
    assert_eq!(
    ...
    let wrapped = ArbiInt::with_precision_wrapping(130_i64, sbp);
    assert_eq!(
        wrapped,
        ArbiInt::from(-126_i64),
...
fn roots_and_serialization() {
    let big = ArbiUint::from(1_000_000_u64);
    let s = big.isqrt().expect("non-negative input");
    assert_eq!(s, ArbiUint::from(1_000_u64), "isqrt of 1e6");
    ...
    let p = ArbiUint::from(1_000_000_u64)
        .next_prime()
        .expect("prime in range");
    assert!(p.is_prime(), "next_prime returns a prime");
    assert_eq!(p, ArbiUint::from(1_000_003_u64), "next prime after 1e6");
    println!("primes:             [OK] next_prime, is_prime");

    let orig = ArbiUint::from(0xDEAD_BEEF_CAFE_u64);
    assert_eq!(
        orig,
        ArbiUint::from_le_bytes(&orig.to_le_bytes()),
        "LE round-trip"
    );
    assert_eq!(
        orig,
        ArbiUint::from_be_bytes(&orig.to_be_bytes()),
        "BE round-trip"
    );
    println!("serialization:      [OK] LE/BE bytes");

    let n = ArbiUint::from(100_u64);
    let d = ArbiUint::from(7_u64);
    let (q, r) = n.div_rem(&d).expect("div_rem");
    assert_eq!(q, ArbiUint::from(14_u64), "quotient");
    assert_eq!(r, ArbiUint::from(2_u64), "remainder");
    assert_eq!(n.div_ceil(&d), ArbiUint::from(15_u64), "div_ceil");
    println!("division:           [OK] div_rem, div_ceil");
}

fn modular_and_misc() {
    let mod7 = ArbiUint::from(7_u64);
    let mx = ArbiUint::from(3_u64);
    let my = ArbiUint::from(5_u64);
    ...
    let f = ArbiUint::factorial(200, Precision::Unlimited);
