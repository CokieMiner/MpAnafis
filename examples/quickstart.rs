...
use arbi_anafis::{ArbiUint, BoundedPrecision, Precision};

fn main() {
    // --- Construction & basic arithmetic ---
    let a = ArbiUint::from(1_000_000_000_000_u64);
    let b = ArbiUint::from(2_u64).pow(80); // 2^80 — too large for u64
    let sum = &a + &b;
    ...
    let x = ArbiUint::with_precision_wrapping(100_000_u64, bp);
    assert_eq!(
        x,
        ArbiUint::from(34_464_u64),
        "u16 wrapping: 100_000 mod 2^16"
    );
    let y = ArbiUint::with_precision_saturating(100_000_u64, bp);
    assert_eq!(
        y,
        ArbiUint::from(65_535_u64),
    ...
    let dec = ArbiUint::from_str_radix("12345678901234567890", 10).expect("valid decimal");
    let hex = ArbiUint::from_str_radix("DEADBEEFCAFE", 16).expect("valid hex");
