...
def extract_allows(text: str, rel_path: str, test_ranges: List[Tuple[int, int]],
                   whole_file_test: bool) -> List[AllowEntry]:
    cleaned = clean_rust_code(text, scrub_attributes=False)
    ...
    for match in ALLOW_RE.finditer(cleaned):
        ...
        entries.append(
            AllowEntry(
                kind=("inner #![allow]" if match.group("prefix") == "#!" else "outer #[allow]"),
                path=rel_path,
                line=cleaned.count("\n", 0, start) + 1,
                lints=lints,
                in_test=(
...
def select_lints(entries: List[AllowEntry], *, lint: Optional[str],
                 mode: str, production_only: bool, test_only: bool) -> List[AllowEntry]:
    wanted = None
    if lint is not None:
        wanted = {lint}
        if "::" not in lint:
...
def run_check_allows(argv: Optional[List[str]] = None) -> int:
    ...
    lint_mode.add_argument("--non-clippy", action="store_true", help="Report only non-Clippy lint allowances")
    lint_mode.add_argument("--all-lints", action="store_true", help="Report both Clippy and non-Clippy lint allowances")
    ...
    for path in rust_source_paths(production_only=args.prod_only):
        ...
        entries.extend(
            extract_allows(text, relative, find_test_ranges(text), is_whole_file_test(relative))
        )
    ...
    for lint in all_lints:
        ...
        print(f"{lint:52s} {prod_counts.get(lint, 0):6d} {test_counts.get(lint, 0):6d}  {location_text}")
    print("-" * 110)
