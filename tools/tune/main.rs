...
fn main() -> Result<(), String> {
    ...
    match &mode {
        ...
        Mode::ScoreSsaMul => {
            worker::print_ssa_mul_score(worker::SsaScoreQuality::Precise);
            Ok(())
        }
        Mode::ScoreSsaMulCoarse => {
            worker::print_ssa_mul_score(worker::SsaScoreQuality::Coarse);
            Ok(())
        }
        Mode::ScoreToom85 => {
...
/// Install the current built-in default for a target that cannot run the tuner.
///
/// Embedded, wasm, and other-ISA targets have no way to execute the measured
/// phases on their own hardware through this tool. This mode only renders the
/// existing default from `build_support/tuning/`; it performs no tuning. The
/// target-selection boundary remains so independently measured defaults can be
/// added later.
fn write_target_profile(target_arch: &str) {
    let profile = profile_for_target(target_arch, conventional_pointer_width(target_arch));
    println!(
        "Installing the built-in default for {target_arch} \
    ...
    while let Some(argument) = arguments.next() {
        let candidate = if let Some(specification) = argument.strip_prefix("--score-mul-pair=") {
            ...
            match argument.as_str() {
                ...
                "--score-ssa-mul" => Mode::ScoreSsaMul,
                "--score-ssa-mul-coarse" => Mode::ScoreSsaMulCoarse,
