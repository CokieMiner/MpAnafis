...
fn tune_knob(session: &mut TuneSession, weights: &[u32], knob: &Knob) -> bool {
    if matches!(knob.domain, ScoreDomain::Ssa) {
        return tune_ssa_knob(session, weights, knob);
    }
...
#[cfg(not(target_pointer_width = "16"))]
fn tune_ssa_knob(session: &mut TuneSession, weights: &[u32], knob: &Knob) -> bool {
    let current = (knob.get)(session.profile);
    println!("\nTuning {} (current {current})", knob.name);
    let mut screened = Vec::with_capacity(knob.candidates.len().saturating_sub(1));
    let Some(coarse_baseline) = session
        .harness
        .score(&session.profile, ScoreDomain::SsaCoarse)
    else {
    ...
    for &candidate in knob.candidates {
        ...
        let mut trial = session.profile;
        (knob.set)(&mut trial, candidate);
        let Some(measurements) = session.harness.score(&trial, ScoreDomain::SsaCoarse) else {
            continue;
        };
    ...
    let Some(baseline) = session.harness.score(&session.profile, ScoreDomain::Ssa) else {
        println!(
    ...
    for candidate in shortlist {
        let mut trial = session.profile;
        (knob.set)(&mut trial, candidate);
        let Some(measurements) = session.harness.score(&trial, ScoreDomain::Ssa) else {
            continue;
        };
        let score = relative_score(&measurements, &baseline, weights);
