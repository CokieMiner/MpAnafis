...
use crate::int::support::{flint_uint_pairs, pin_flint_to_one_thread, verify_flint_matches_arbi};
use crate::int::{
    ladders::{EXTENDED_GCD, THEORY},
    support::{SAMPLE_COUNT_WIDE, SAMPLE_SIZE_FAST, SAMPLE_SIZE_WIDE, arbi_uint_pairs},
};
...
mod gcd {
    ...
    fn flint(bencher: divan::Bencher, bits: usize) {
        ...
        let arbi_inputs = arbi_uint_pairs(bits);
        assert_eq!(inputs.len(), arbi_inputs.len(), "paired benchmark batch");
        for ((left, right), (arbi_left, arbi_right)) in inputs.iter().zip(&arbi_inputs) {
            verify_flint_matches_arbi(arbi_left, left);
            verify_flint_matches_arbi(arbi_right, right);
            let actual = left.gcd(right);
            let expected = arbi_left.gcd(arbi_right);
