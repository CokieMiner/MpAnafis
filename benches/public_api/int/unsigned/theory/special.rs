...
use crate::int::support::{
    FlintInt, flint_odd_uint, flint_uint, pin_flint_to_one_thread, verify_flint_matches_mp,
};
use crate::int::support::{
...
mod factorial {
    ...
    fn mp(bencher: divan::Bencher, terms: u32) {
        bencher.bench_local(|| {
            let _output = black_box(MpUint::factorial(black_box(terms), Precision::Unlimited));
        });
    }
    ...
    fn flint(bencher: divan::Bencher, terms: u32) {
        ...
        let expected = MpUint::factorial(terms, Precision::Unlimited);
...
mod jacobi_symbol {
    ...
    fn flint(bencher: divan::Bencher, bits: usize) {
        pin_flint_to_one_thread();
        let mp_value = mp_uint(bits, 42);
        let mp_modulus = MpUint::from_str_radix(&odd_hex(bits, 9_999), 16)
            .expect("generated modulus must parse as MpUint");
        let value = flint_uint(bits, 42);
        let modulus = flint_odd_uint(bits, 9_999);
        verify_flint_matches_mp(&mp_value, &value);
        verify_flint_matches_mp(&mp_modulus, &modulus);
        assert_eq!(
            value.jacobi(&modulus),
            i32::from(
                mp_value
                    .jacobi_symbol(&mp_modulus)
                    .expect("the generated modulus is positive and odd")
            ),
            "FLINT and Mp must compute the same Jacobi symbol"
        );
        bencher.bench_local(|| {
