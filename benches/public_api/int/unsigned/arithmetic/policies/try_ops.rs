...
//! `ArbiError` variant as the Arbi method so both cells include equivalent
//! `Result` construction.
...
#[cfg(all(target_arch = "x86_64", target_pointer_width = "64"))]
fn rug_try_add(left: &Integer, right: &Integer, width: u32) -> Result<Integer, ArbiError> {
    let result = Integer::from(Add::add(left, right));
    if result.significant_bits() > width {
        Err(ArbiError::Overflow)
    } else {
        Ok(result)
    }
}
...
#[cfg(all(target_arch = "x86_64", target_pointer_width = "64"))]
fn rug_try_sub(left: &Integer, right: &Integer, _width: u32) -> Result<Integer, ArbiError> {
    let result = Integer::from(Sub::sub(left, right));
    if result.is_negative() {
        Err(ArbiError::Underflow)
    } else {
        Ok(result)
    }
}
...
#[cfg(all(target_arch = "x86_64", target_pointer_width = "64"))]
fn rug_try_mul(left: &Integer, right: &Integer, width: u32) -> Result<Integer, ArbiError> {
...
#[cfg(all(target_arch = "x86_64", target_pointer_width = "64"))]
fn rug_try_div(left: &Integer, right: &Integer, _width: u32) -> Result<Integer, ArbiError> {
    if right.is_zero() {
        Err(ArbiError::DivisionByZero)
    } else {
...
#[cfg(all(target_arch = "x86_64", target_pointer_width = "64"))]
fn rug_try_rem(left: &Integer, right: &Integer, _width: u32) -> Result<Integer, ArbiError> {
    if right.is_zero() {
        Err(ArbiError::DivisionByZero)
    } else {
