...
use super::{SAMPLES, arbi_uint, arbi_uint_pairs_with_widths, odd_hex, random_hex};
#[cfg(all(target_arch = "x86_64", target_pointer_width = "64"))]
...
#[must_use]
pub fn arbi_div_pairs_2n_n(bits: usize) -> Vec<(ArbiUint, ArbiUint)> {
    arbi_uint_pairs_with_widths(bits.saturating_mul(2), bits)
}

/// The Rug counterpart of [`arbi_div_pairs_2n_n`].
#[cfg(all(target_arch = "x86_64", target_pointer_width = "64"))]
#[must_use]
...
#[must_use]
pub fn arbi_div_pairs_3n2_n(bits: usize) -> Vec<(ArbiUint, ArbiUint)> {
    arbi_uint_pairs_with_widths(bits.saturating_add(bits >> 1), bits)
}

/// The Rug counterpart of [`arbi_div_pairs_3n2_n`].
#[cfg(all(target_arch = "x86_64", target_pointer_width = "64"))]
#[allow(
    clippy::arithmetic_side_effects,
    reason = "benchmark bit width calculations are exact and guaranteed not to overflow"
...
#[must_use]
pub fn arbi_div_pairs_same_limbs_ge_2(bits: usize) -> Vec<(ArbiUint, ArbiUint)> {
    (0..SAMPLES)
        .map(|index| {
            let left = arbi_uint(bits, 42_u32.wrapping_add(index));
            let mut right = arbi_uint(bits, 1_337_u32.wrapping_add(index));
            right >>= 2;
            if right.is_zero() {
                right = ArbiUint::from(1_u32);
            }
            (left, right)
        })
        .collect()
...
/// The Rug counterpart of [`arbi_div_pairs_same_limbs_ge_2`].
#[cfg(all(target_arch = "x86_64", target_pointer_width = "64"))]
#[allow(
    clippy::arithmetic_side_effects,
    reason = "benchmark operand construction is guaranteed not to overflow"
...
/// guard clause rather than the extended GCD. The search is over `ArbiUint` so
...
pub fn coprime_hex_pair(bits: usize) -> (String, String) {
    ...
    let modulus = ArbiUint::from_str_radix(&modulus_hex, 16)
        .expect("generated odd hexadecimal must parse as ArbiUint");
    let one = ArbiUint::one();
    let mut candidate_seed = 42_u32;

    loop {
        let candidate_hex = random_hex(bits, candidate_seed);
        let candidate = ArbiUint::from_str_radix(&candidate_hex, 16)
...
#[must_use]
pub fn arbi_known_primes(bits: usize) -> Vec<ArbiUint> {
    (0..SAMPLES)
        .map(|index| {
            arbi_uint(bits, 42_u32.wrapping_add(index))
                .next_prime()
...
/// The Rug counterpart of [`arbi_known_primes`].
#[cfg(all(target_arch = "x86_64", target_pointer_width = "64"))]
#[must_use]
...
#[must_use]
pub fn arbi_semiprimes_no_small_factors(bits: usize) -> Vec<ArbiUint> {
    let half_bits = bits >> 1;
    (0..SAMPLES)
        .map(|index| {
            let p1 = arbi_uint(half_bits.max(32), 42_u32.wrapping_add(index))
                .next_prime()
                .expect("next_prime returns Some for valid benchmark bit widths");
            let p2 = arbi_uint(half_bits.max(32), 1_337_u32.wrapping_add(index))
                .next_prime()
                .expect("next_prime returns Some for valid benchmark bit widths");
            &p1 * &p2
        })
...
/// The Rug counterpart of [`arbi_semiprimes_no_small_factors`].
#[cfg(all(target_arch = "x86_64", target_pointer_width = "64"))]
#[allow(
    clippy::arithmetic_side_effects,
    reason = "benchmark operand construction is guaranteed not to overflow"
...
#[must_use]
pub fn arbi_true_squares(bits: usize) -> Vec<ArbiUint> {
    let half_bits = bits >> 1;
    (0..SAMPLES)
        .map(|index| {
            let root = arbi_uint(half_bits.max(32), 42_u32.wrapping_add(index));
            &root * &root
        })
        .collect()
}

/// The Rug counterpart of [`arbi_true_squares`].
#[cfg(all(target_arch = "x86_64", target_pointer_width = "64"))]
#[allow(
    clippy::arithmetic_side_effects,
    reason = "benchmark operand construction is guaranteed not to overflow"
...
#[must_use]
pub fn arbi_square_plus_one(bits: usize) -> Vec<ArbiUint> {
    arbi_true_squares(bits)
        .into_iter()
        .map(|mut square| {
            square += ArbiUint::from(1_u32);
            square
        })
        .collect()
}

/// The Rug counterpart of [`arbi_square_plus_one`].
#[cfg(all(target_arch = "x86_64", target_pointer_width = "64"))]
#[allow(
    clippy::arithmetic_side_effects,
    reason = "benchmark operand construction is guaranteed not to overflow"
