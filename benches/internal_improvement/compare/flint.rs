...
//! which is a distinct quasi-linear competitor to our recursive Fermat-ring
//! Schönhage-Strassen path. That makes FLINT, not GMP, the relevant adversary
//! above the transform crossover.
//!
...
/// Asserts that Mp, GMP, and FLINT all index limbs at the same width.
...
impl FlintThreadBudget {
    ...
    pub fn new(workers: usize) -> Self {
        let workers_result = i32::try_from(workers);
        assert!(
            workers_result.is_ok(),
            "FLINT worker budget must fit in i32"
        );
        // SAFETY: the assertion above validates the conversion before changing
        // FLINT's process-global worker budget.
        let workers_i32 = unsafe { workers_result.unwrap_unchecked() };
