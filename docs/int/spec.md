# Arbi integer crate specification
...
- After that, `ArbiInt`, `ArbiUint`, `ArbiRational`, and `ArbiFloat` carry enough precision metadata for normal arithmetic, conversions, parsing, formatting, and generic APIs to behave predictably.
- Passing an Arbi value into a function must not require the callee to separately receive precision unless that function is explicitly constructing new independent values or requesting a different precision.

Interoperability goal:
- Arbi numeric types should participate in Rust's standard numeric ecosystem as far as soundly possible: `From`, `TryFrom`, `FromStr`, `Display`, `core::ops`, iterator `Sum`/`Product`, `num-traits`, `serde`, `rand`, and cross-type comparisons/conversions.
- Generic functions that accept primitive integers, floats, rationals, or common numeric traits should be able to accept Arbi types whenever the trait contract can be satisfied without lying.
...
- Rust has no implicit argument conversion. A function declared as `fn f(x: f64)` will not accept `ArbiFloat` automatically. The guarantee targets generic APIs (`T: Num`, `T: Into<ArbiFloat>`, `T: TryInto<ArbiInt>`, crate umbrella traits, etc.) and explicit conversions at concrete primitive boundaries.
...
- The Arbi design accepts an ambient precision policy layer to reduce precision plumbing, while still exposing explicit `_with_precision` and `try_*` APIs for deterministic library code, tests, and low-level control.
...
Arbi follows a numeric tower for mixed-type operations:

```text
ArbiUint / ArbiInt -> ArbiRational -> ArbiFloat
```

- Integer with integer returns an integer type when the operation is closed over that type.
- Integer with rational promotes to `ArbiRational`.
- Any operation involving `ArbiFloat` promotes to `ArbiFloat`.
- Primitive integers promote into the matching Arbi integer type unless the other operand is rational or float.
- `f32` and `f64` promote to `ArbiFloat` by decoding the exact IEEE-754 value first, then rounding to the resolved ArbiFloat target precision if necessary.
- Rust has no implicit conversion at concrete argument boundaries; this promotion policy applies to Arbi operator impls, constructors, explicit conversions, and generic helper traits.
...
- Conversion from exact values (`ArbiInt`, `ArbiUint`, `ArbiRational`) into `ArbiFloat` uses the active float target precision and rounding mode.

## 3. Precision System

`ArbiInt` and `ArbiUint` support unlimited and bounded precision modes.
...
2. **Bounded precision**:
   ...
   - `ArbiUint` behaves as an unsigned N-bit integer ($0 \le x \le 2^N - 1$).
   - `ArbiInt` behaves as a signed N-bit two's-complement integer at the public API boundary ($-2^{N-1} \le x \le 2^{N-1} - 1$).
...
1. **Value precision metadata:**
   Existing Arbi values carry their own precision. Integer and rational arithmetic derives result precision from operand metadata. Ambient precision does not cap or rewrite results of operations on existing values.
...
```
Plus `ParseArbiIntError` / `ParseArbiUintError`.

## 5. Primitive Parity & Safe Defaulting
Bounded operations in Arbi preserve mathematical correctness as strictly as possible:
...
- For primitive-like wrapping semantics, a separate wrapper type (e.g., `WrappingArbiUint`) can be used to opt into wrapping operations. There is no `primitive-overflow-semantics` feature flag, ensuring consistent ecosystem behavior.
...
- `checked_* -> Option<Output>`: arithmetic/domain failure only (`Overflow`, `Underflow`, `DivisionByZero`, invalid signed `MIN` operation, invalid root/log input). No allocation, width, parsing, or feature errors are encoded in `Option`. In `no_alloc` or fixed-buffer configurations, if a `checked_*` operation cannot allocate or cannot fit inline, it returns `None`. Use `try_*` to distinguish between arithmetic failure and `ArbiError::AllocationRequired`.
- `try_* -> Result<Output, ArbiError>`: failures needing explanation (`WidthRequired`, `AllocationRequired`, `ShiftTooLarge`, `FactorizationRequired`, `InvalidRadix`, etc.).
- `wrapping_*`, `overflowing_*`, `saturating_*`, `strict_*`, and `unchecked_*`: follow primitive integer naming and tuple shapes. Because Arbi bounded operators already panic on overflow in all build modes, `strict_*` methods are mostly explicit aliases for plain checked-then-panic arithmetic. They are provided for primitive API parity and for codebases that want overflow intent visible at call sites.
...
- For always-wrapping ergonomic environments, explicitly use wrappers like `WrappingArbiUint(pub ArbiUint)` or `WrappingArbiInt(pub ArbiInt)`.
...
- Standard arithmetic operators grow to fit. `ArbiUint` subtraction panics on underflow.
...
- `checked_sub` on `ArbiUint` returns `None` under zero; `saturating_sub` clamps to zero.

## 6. Type Definitions & Internal Layout
### 6.1 Core Layout
- **Types**: `InternalArbiUint` (Magnitude) and `InternalArbiInt` (Signed)
- **Precision Enum**: `#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)] pub enum Precision { Unlimited, Bounded(BoundedPrecision) }`.
- **Implementation Strategy**:
  - **Storage**: Small-Inline Representation (`enum UintRepr { Inline { len: u8, limbs: [usize; 4] }, Heap(alloc::vec::Vec<usize>) }`). The limb count is fixed at four; inline bit capacity is 64, 128, or 256 bits on 16-, 32-, or 64-bit targets. Both `ArbiUint` and `ArbiInt` are inherently `Send + Sync`.
  ...
  - **Sign**: `InternalArbiInt` stores a positive-sign flag beside the unsigned magnitude.
  - **Normalization of -0**: `ArbiInt` enforces strict normalisation. Magnitude zero always has the positive sign. `-0` is never observable.
  - **HashMap Collisions**: Because `Eq` and `Hash` are value-based and ignore precision metadata, `ArbiUint::with_precision(5, 8)` and `ArbiUint::from(5u8)` compare equal and behave as the same key in a `HashMap`. For cases where precision distinction is needed in hashing, a `PrecisionSensitive<T>` wrapper may be exposed or documented.
...
- `MINUS_ONE`: Exposed only on `ArbiInt` as a constructor (`fn minus_one() -> Self`).
...
- Bounded signed invariant: A bounded `ArbiInt` strictly enforces $-2^{N-1} \le x \le 2^{N-1} - 1$. The edge case $N = 1$ enforces $-1 \le x \le 0$.
...
> - **Only on `ArbiInt` (Signed):** `abs`, `signum`, `is_negative`, `is_minus_one`.
> - **Omitted from `ArbiUint` entirely:** `is_positive`, `is_negative`. On `ArbiInt`, `is_positive` is defined as `self > 0`. Pure bitwise ops natively operate here. `sqrt` and `isqrt` exist only on `ArbiUint`. `ArbiInt` only provides `checked_isqrt` (returns `None` for negative values).
> - **Shared / Delegated:** Most heavy math (`add`, `mul`, `pow`, `div_rem`, `gcd`) is implemented natively on `ArbiUint` and wrapped by `ArbiInt` to manage the sign. Number theory functions on `ArbiInt` operate on the absolute magnitude unless otherwise noted. Functions undefined for non-positive inputs (primality, totient, Jacobi with non-positive modulus, etc.) return `NonPositiveInput` or panic according to the method family. `is_zero`, `is_one`, `div_floor`, `div_ceil`, `div_trunc` are shared.
...
- `ArbiUint::isqrt`: Floor of the unsigned square root. It already returns `Option`, so an identical unsigned `checked_isqrt` alias is intentionally omitted. `ArbiInt::checked_isqrt` remains meaningful because negative inputs return `None`.
- `div_rem`, `div_euclid`, `rem_euclid`, `div_trunc`, `rem_trunc`, `div_floor`, `mod_floor`, `div_ceil`. (Note: `ArbiInt::bounded_i8_min() / ArbiInt::from(-1)` panics for `/`, returns `None` for `checked_div`, wraps for `wrapping_div`).
- `is_divisor_of` / `is_divisible_by` (Divisibility predicate).
- `abs_diff` (Absolute difference): Returns `ArbiUint`. Result precision is `Bounded(max(width_a, width_b))` for bounded unsigned inputs, and `Bounded(max(width_a, width_b) + 1)` for signed inputs to hold the maximum possible difference without overflow.
- `abs_sub` (Positive difference, `ArbiInt` only): Returns `max(0, self - other)` as an `ArbiInt`. **This is not the absolute difference** — it clamps at zero rather than taking a magnitude, so `(-5).abs_sub(3) == 0` where `(-5).abs_diff(3) == 8`. It exists because `num_traits::Signed` requires it under this name; `abs_diff` is the method to reach for otherwise. The two are kept adjacent in the documentation precisely because the names invite the confusion.
- `div_2exp` / `mul_2exp` / `mod_floor_2exp` / `rem_trunc_2exp`: On `ArbiInt`, `mod_floor_2exp` follows floor division semantics (the result is always non-negative), whereas `rem_trunc_2exp` carries the dividend's sign.
- `widening_mul` / `carrying_mul` / `carrying_mul_add`: Returns `ArbiUint` / `ArbiInt`. (Note: `checked_mul` on `Unlimited` may return `None` if allocation fails, per normal memory handling semantics, though it generally never panics on pure overflow).
- `try_widening_mul`: Returns `Result<(ArbiUint, ArbiUint), ArbiError>` (lower, upper). On unlimited `ArbiUint`, it returns `ArbiError::WidthRequired` rather than panicking.
...
- `assign_sub` on `ArbiUint` returns `bool` reporting underflow, because an unsigned destination cannot represent a negative difference. The `ArbiInt` form returns `()`.

### Sign & Properties
- `abs`, `abs_assign`, `unsigned_abs`, `signum`, `apply_sign`. `signum(0)` returns an `ArbiInt` with value 0, preserving its input precision. In `crypto` mode, `apply_sign` uses `conditional_select`, and `Sign` is `#[repr(i8)]` to enable branchless execution.
- `is_zero`, `is_one`.
- `is_positive`, `is_negative`, `is_minus_one` (Only on `ArbiInt`).
- `is_even`, `is_odd`.
...
For bounded signed bitwise operations, `ArbiInt` is first interpreted as an N-bit two's-complement value, operated on, then converted back into canonical sign-magnitude form.

**Behavior Matrix**:
| Operation | `ArbiUint` unlimited | `ArbiInt` positive unlimited | `ArbiInt` negative unlimited | Bounded |
|---|---|---|---|---|
...
- `not`: `Not` trait panics on `ArbiUint` unlimited since it requires width. It is fully defined on `ArbiInt` unlimited, where the sign supplies the infinite extension.
  - `not_with_width(bits) -> Option<Self>`: complement within an explicit width. `None` when `bits` is zero. This is the operation `!` cannot express on an unlimited `ArbiUint`.
  - `try_not() -> Result<Self, ArbiError>`: complement within the value's *own* bounded precision. `ArbiError::WidthRequired` when the precision is unlimited.
...
- `shl`, `shr`, `shl_assign`, `shr_assign`: For `ArbiInt` unlimited, arithmetic `shr` on negatives preserves sign and extends infinitely (i.e. `-1 >> n == -1`). For massive shifts without allocation panic, use `try_shl_big(&huge_shift)` / `try_shr_big`.
- `rotate_left`, `rotate_right`, `reverse_bits`: Require bounded precision. `swap_bytes`: On `ArbiUint` works with any precision (uses significant bits on unlimited); on `ArbiInt` returns `None` for unlimited (width-dependent).
...
- `find_first_set_bit`, `find_first_zero_bit`, `find_next_set_bit(from: u64)`, `find_next_zero_bit(from: u64)` (Note: for unlimited `ArbiUint`, `find_next_zero_bit` returns `from` if `from` is past the highest set bit).
...
- `remove_factor`: Base 0 or 1 returns `ArbiError::InvalidInput`.
...
- `bell`, `partition`: For large values, uses Hardy-Ramanujan-Rademacher formula which requires `ArbiFloat` from the unified package.
- `euler_number`.
- `bernoulli` / `harmonic_number`: Provided natively on integer API, computing and returning `ArbiRational`. This requires the `combinatorics` feature to optimize code size.
- `tetration` / `hyperoperation`: `a^^0 = 1`.

### Constructors, Conversions, Parsing & Formatting
- **Explicit Constructor Matrix**:
  - `ArbiUint::new(value)`
  - `ArbiUint::with_precision_checked(value, bits) -> Result<Self, ArbiError>`
  - `ArbiUint::with_precision_wrapping(value, bits) -> Self`
  - `ArbiUint::with_precision_saturating(value, bits) -> Self`
  - `ArbiUint::zero_with_precision(bits) -> Self`
  - *(Same applies to `ArbiInt`)*
...
- `write_be_bytes(buf: &mut [u8]) -> Result<(), ArbiError>`, `write_le_bytes`, `write_native_endian_bytes`: Non-allocating variants that write into a provided buffer (fails if buffer is too small).
...
- `From` / `Into` / `TryFrom` for primitive integers. `TryFrom<ArbiRational>` returns `ArbiError::IntegerConversionLoss` on fractional parts. 
- **Float Conversions**: `TryFrom<f64>` truncates towards zero (fails on NaN/Infinity). For `ArbiUint`, any finite input less than 0 (e.g. `-0.5`) returns `NegativeInput` before truncation (but `-0.0` is `0`). Additional explicits: `try_from_f64_exact`, `_trunc`, `_floor`, `_ceil`. `FromPrimitive::from_f64` returns `None` for NaN/Infinity/negative (except `-0.0`), truncating towards zero.
- **Cross-Type Conversions**: `From<ArbiUint> for ArbiInt` (infallible). `TryFrom<ArbiInt> for ArbiUint` (fails with `NegativeInput` if negative).
...
- `Eq`, `PartialEq`, `Ord`, `PartialOrd`, `Hash`. `Ord` is implemented within each concrete type. Cross-type numeric comparisons are implemented exclusively through `PartialEq` and `PartialOrd` (e.g., `ArbiInt(5) == ArbiUint(5)` is `true`).
...
- `serde`: The target serialized format uses fixed-width `u64` words rather than native `usize` limbs: `ArbiUint` serializes as `{ precision: u64, limbs_le: [u64] }` and `ArbiInt` as `{ precision: u64, sign: i8, limbs_le: [u64] }`. Equality/hash identity is numeric-value identity, not serialization identity.
...
- `num-bigint` / `num-traits` compat: Provides `From<num_bigint::BigUint> for ArbiUint`, `From<num_bigint::BigInt> for ArbiInt`, and inverses.
...
a caller cannot ask an `ArbiUint` or `ArbiInt` what precision it carries. The
...
4. **Phase 4 (Unified)**: Rational and float integration. (Note: `bernoulli` and `harmonic_number` return `ArbiRational`, so they belong here).
...
- `ArbiInt` enforces strictly positive sign for zero (no `-0`).
...
- **Cross-Type**: `ArbiInt(-1) < ArbiUint(0)`, large unsigned to signed conversions.
...
graph TD
    ...
    subgraph L6: Conversions & IO
        ...
        ArbiRational["ArbiRational Output"]
        ArbiFloat["ArbiFloat Dependency"]
    end
    DivRem --> Str
    ILog --> Str
    Shift --> Bytes
    Props2 --> Float
    RationalComb --> ArbiRational
    Partition --> ArbiFloat
```
